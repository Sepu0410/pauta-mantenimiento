    ...
    try {
      ...
      function addHeaderAndFooter() {
        ...
        for (var i = 1; i <= pageCount; i++) {
          ...
          doc.text(eqText + ' | ' + data.fecha, margin, pageHeight - 8);
          doc.text('Página ' + i + ' de ' + pageCount, pageWidth - margin, pageHeight - 8, { align: 'right' });
        }
        doc.setPage(doc.internal.getCurrentPage().pageNumber || 1);
      }
      ...
      doc.text('PAUTA DE MANTENIMIENTO', pageWidth / 2, y, { align: 'center' });
      y += 7;

      doc.setFontSize(13);
      doc.setTextColor(47, 117, 181);
      doc.text('LINCOLN VANTAGE 500', pageWidth / 2, y, { align: 'center' });
      y += 10;
      ...
      doc.setFontSize(10);
      doc.setTextColor(23, 54, 93);
      doc.text('ACTIVIDADES DE MANTENIMIENTO', margin, y);
      y += 4;
      ...
      if (data.spareParts && data.spareParts.length > 0) {
        ...
        doc.setFontSize(10);
        doc.setTextColor(23, 54, 93);
        doc.text('REPUESTOS ADICIONALES', margin, y);
        y += 4;
      ...
      if (hasPhotos) {
        ...
        data.photos.forEach(function (p) {
          if (p.data) {
            ...
            doc.roundedRect(px, py, photoW, photoH, 2, 2, 'FD');
            ...
            doc.text(p.label, px + photoW / 2, py + photoH - 3, { align: 'center' });
      ...
      for (var si = 0; si < data.signatures.length; si++) {
        ...
        doc.setDrawColor(180, 180, 180);
        doc.roundedRect(sx, sy, sigW, sigH, 2, 2, 'S');
        ...
        doc.setFontSize(8);
        doc.setTextColor(80, 80, 80);
        doc.text(sig.label, sx + sigW / 2, sy + sigH + 5, { align: 'center' });
      }
